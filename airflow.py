...
with DAG(
  ...
  mapreduce_classic = BashOperator(
    ...
    bash_command="""hadoop jar {{ params.dags_home }}/project_files/zestaw4.jar AvgSizeStations {{ params.input_dir }}/datasource1/title.principals.tsv {{ params.output_mr_dir }}
    hadoop fs -getmerge {{ params.output_mr_dir }} result1.tsv
    hadoop fs -copyFromLocal result1.tsv {{params.output_mr_dir}}
    """,
  )
...
... """,
  ...
  hive = BashOperator(
    ...
    bash_command="""hive -f {{ params.dags_home }}/project_files/transform5.hql \
      ...
      -hiveconf input_dir3={{ params.output_mr_dir }} \      -hiveconf output_dir6={{ params.output_dir }}""",
  )
  ...
  pig = BashOperator(
    ...
    bash_command="""export PIG_CLASSPATH=/etc/hadoop/conf.empty:/etc/tez/conf
      pig -x tez -f {{ params.dags_home }}/project_files/name_schema.pig \
